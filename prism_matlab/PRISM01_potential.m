function [emdSTEM] = PRISM01_potential(atoms,cellDim)
rng(1);  % for repeatable testing

% Colin Ophus - 2020 Sept
% 11 - calculate the projected potential from a collection of an orthogonal
...
flagAvgFP = true;
pixelSize = 0.10;            % Realspace pixel size.
emdSTEM.potBound = 2;       % Radial distance to integrate atomic potentials. [Ang]
emdSTEM.potSamplingZ = 0.1; % Z sampling density for integrating atomic potentials. [Ang]
emdSTEM.potBandLimit = [0.75 0.95]; % [min max] units of Nyquist, so 0 to 1
emdSTEM.numFP = 1;          % Number of frozen phonon configurations.
...
u = ones(118,1) * 0.10;      % RMS atomic displacements (Debye waller coefficients).
...
% Simulation size
f = 2*2*emdSTEM.interpolationFactor;  % I can't decide between 2 and 4 here
